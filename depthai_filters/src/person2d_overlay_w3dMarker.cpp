...
void Person2DOverlay3d::overlayCB3d(const sensor_msgs::ImageConstPtr& preview, const vision_msgs::Detection2DArrayConstPtr& detections) {
    ...
    for(auto& detection : detections->detections) {
        ...
        /*     dronePose->pose.pose.orientation.x, dronePose->pose.pose.orientation.y, dronePose->pose.pose.orientation.z, dronePose->pose.pose.orientation.w); */
        ...
        const double yaw = (detection.bbox.center.x - previewMat.cols / 2) / previewMat.cols * field_of_view;
        // Change yaw to quaternion - LB
        tf::Quaternion q_detection;
        q_detection.setRPY(0, 0, yaw);
        ...
        const double dx = drone_x - old_drone_x;
        const double dy = drone_y - old_drone_y;
        const double dz = drone_z - old_drone_z;
